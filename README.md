...
Vlog 工作台按用户手动添加的片段组工作：每组可以选择 1–4 张图片走 AI 图生视频（单图和多图都支持）、1 张图片走本地动态效果，或直接复用历史生成视频/本地视频。所有组最后统一选择转场并在浏览器内合成。AI 组的一句话补充会传给每个 AI 片段；历史记录中的单图 AI 视频与多图 AI 视频都可以作为视频素材复用。

Vlog 还提供「本地图片动效」模式：选择 1 张本地图片，套用缓慢推进、缓慢拉远、向左/向右平移或轻微漂移模板，设置片段时长后生成 MP4。图片和合成过程只在当前浏览器处理，不创建 AI 任务；最终混合成片会保存到服务端历史。
...
- 已完成的 Vlog 可从历史记录点击「编辑 Vlog」，或在项目详情点击「编辑副本」。图片分组、风格、画幅、描述和转场会回填到新草稿；同一浏览器保存的本地图片/视频也会尝试恢复
