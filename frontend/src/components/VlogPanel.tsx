...
export default function VlogPanel({ config, styles }: Props) {
  ...
  const [result, setResult] = useState<{ url: string; duration: number; saved: boolean } | null>(null)
  ...
  const canMergeReadyProject = !!project && project.status === 'ready_to_merge' && allValid && !busy
  ...
  const uploadAiImages = async (id: string, files: File[]) => {
    if (files.length < 1 || files.length > 4) return setError('AI 图生视频组请选择 1–4 张图片')
    setBusy(true)
    setError('')
    try {
      const uploaded = await uploadVlogImages(files)
      updateGroup(id, (group) => ({ ...group, images: uploaded.images, duration: aiDurationForCount(uploaded.images.length) }))
      setRatio(uploaded.ratio)
    } catch (cause) {
  ...
  const mergeFinal = async (current: VlogProject | null) => {
    ...
    try {
      ...
      if (current) {
        try {
          const completed = await completeVlog(current.id, merged.blob, merged.duration ?? totalDuration)
          setProject(completed)
          localStorage.removeItem(VLOG_TIMELINE_KEY)
          saved = true
        } catch (cause) {
          saveError = `成片已生成，但保存到 Vlog 项目失败：${(cause as Error).message}`
        }
      } else {
        try {
          await saveMergedVideo(merged.blob, {
            ...
            totalDuration: merged.duration ?? totalDuration,
          })
          saved = true
        } catch (cause) {
          saveError = `成片已生成，但保存到历史记录失败：${(cause as Error).message}`
        }
      }
      if (saveError) setError(saveError)
      setResult({ url, duration: merged.duration ?? totalDuration, saved })
    } catch (cause) {
      setError((cause as Error).message)
    } finally {
      temporaryUrls.forEach((url) => URL.revokeObjectURL(url))
        ...
        <section className="scene-groups" aria-label="Vlog 片段组">
          {groups.map((group, index) => (
            <article className={`scene-group scene-group-${group.mode}`} key={group.id}>
              ...
              {group.mode === 'ai' && <div className="group-content"><input ref={(element) => { inputRefs.current[`${group.id}-ai`] = element }} type="file" accept="image/png,image/jpeg,image/webp" multiple hidden onChange={(event) => { const files = Array.from(event.target.files ?? []); if (files.length) uploadAiImages(group.id, files); event.target.value = '' }} />{group.images.length === 0 ? <button className="group-dropzone" type="button" disabled={!canEditGroupStructure} onClick={() => inputRefs.current[`${group.id}-ai`]?.click()}><span>＋</span><strong>选择这一组的 1–4 张图片</strong><small>这些图片只会生成一个 AI 视频片段</small></button> : <div className="group-image-grid">{group.images.map((image, imageIndex) => <figure key={image.image_path} className="group-image"><img src={image.url} alt={`第 ${index + 1} 组图片 ${imageIndex + 1}`} /><figcaption><span>{imageIndex + 1}</span><button className="icon-btn" type="button" title="删除图片" aria-label="删除图片" disabled={!canEditGroupStructure} onClick={() => updateGroup(group.id, (current) => { const images = current.images.filter((_, itemIndex) => itemIndex !== imageIndex); return { ...current, images, duration: aiDurationForCount(images.length) } })}>×</button></figcaption></figure>)}{canEditGroupStructure && <button className="add-image-tile" type="button" onClick={() => inputRefs.current[`${group.id}-ai`]?.click()}>＋<span>重新选择图片</span></button>}</div>} {group.images.length >= 1 && <div className="group-meta"><span>{group.images.length} 张图片 · 1 次 AI 调用 · 约 {aiDurationForCount(group.images.length)} 秒</span><button className="link-btn" type="button" disabled={!canEditGroupStructure} onClick={() => inputRefs.current[`${group.id}-ai`]?.click()}>重新选择</button></div>}</div>}
